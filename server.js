...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    // 1. Check knowledge base for matching Q&A
    ...
    try {
      ...
      if (knowledge.length > 0) {
        ...
        const relevant = knowledge.filter(k => {
          ...
          const a = (k.answer || '').toLowerCase();
          const userQ = lastQuestion.toLowerCase();
          // Match if any word from user question appears in knowledge
          const words = userQ.split(' ').filter(w => w.length > 3);
          return words.some(w => q.includes(w) || a.includes(w));
        });
        if (relevant.length > 0) {
          knowledgeContext = '\n\n## IMPORTANT: Use this verified knowledge in your answer:\n' +
            relevant.map(k => k.question
              ? `Q: ${k.question}\nA: ${k.answer}`
              : k.answer
            ).join('\n\n');
        }
      }
    } catch(e) { /* continue without knowledge */ }
...
app.get('/api/knowledge', async (req, res) => {
  try {
    const data = await supabase('/knowledge?select=*&order=created_at.desc');
    res.json(data);
...
app.post('/api/knowledge', async (req, res) => {
  try {
    ...
    const data = await supabase('/knowledge', 'POST', req.body);
    res.json(data);
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/suggestions', async (req, res) => {
  try {
    const data = await supabase('/suggestions?select=*&order=created_at.desc');
    res.json(data);
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/suggestions', async (req, res) => {
  try {
    const data = await supabase('/suggestions', 'POST', req.body);
    res.json(data);
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.patch('/api/suggestions/:id', async (req, res) => {
  try {
    ...
    if (status === 'approved' && answer) {
      await supabase('/knowledge', 'POST', {
        type: 'qa',
        question: req.body.question,
        answer: answer,
        topic: 'user-suggestion'
      });
    }
    res.json({ success: true });
  } catch(err) { res.status(500).json({ error: err.message }); }
